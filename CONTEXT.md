...
- `2026-09-24` test-coverage (GoodJob::Execution model) — pr-opened-green — fork PR #7 (spec/app/models/good_job/execution_spec.rb, 18 examples over the full public API; added +18/-0 in 1 file). Fork CI: all 21 checks green (Lint, CodeQL, Analyze x2, Tests for Development and Demo, rails_6.1/7.0/7.1/7.2/8.0/8.1/rails_head x Ruby 3.0/3.1/3.2/3.3/3.4/4.0/jruby-10.0), mergeable_state clean. One CI iteration caught a timestamp-precision assertion (#last_status_at eq a raw Ruby Time fails on Rails 7.x where PG truncates to microseconds); fixed to compare against the reloaded DB finished_at and re-ran the whole matrix green. Local: spec green + rubocop clean + full spec/lib spec/app 780 examples, 0 failures.

- `2026-09-24` docs-grounded (README Job priority: stale `smaller_number_is_higher_priority` migration instruction) — pr-opened — fork PR #8 (branch fix-removed-priority-config-doc; +1/-1 in README.md). The option was removed in v4.1.1 (upstream #1453); dequeue ordering is fixed `priority ASC NULLS LAST`, so no opt-in option exists. Deduped: no open/merged upstream PR touches this line. Verified current upstream: option absent from lib/good_job/configuration.rb. Local: `bundle exec rubocop` clean + repo-style `bundle exec mdl --style .mdstyle.rb README.md` clean; full `bin/rspec` 905 examples w/ 7 env-specific failures (ProbeServer network-bind `Net::OpenTimeout` to sandbox 10.14.0.2 + one system jobs_spec capybara render-timing) -- none touched by a README text change. Fork CI: 19/21 checks green; 2 (rails_8.0 r3.2, rails_head r3.3) fail only at `actions/checkout@v7` with git `server certificate verification failed` -- pre-checkout SSL infra flake, unrelated to docs-only diff.

## Mined gaps (discovered, not yet attempted)
- `2026-09-24` test-coverage (GoodJob::Execution model) — no dedicated model spec exists for Execution; public API (#number, #queue_latency, #runtime_latency, #last_status_at, #status, #interrupted_duration, #display_serialized_params, #filtered_error_backtrace) drives dashboard views and jobs#executions. Proposed: add spec/app/models/good_job/execution_spec.rb. Dedupe: no upstream execution_spec.rb; no open/merged upstream PR covering it. — status: attempted (pr-opened, fork PR #7)
